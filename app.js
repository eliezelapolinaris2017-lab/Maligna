/* =========================
   Inventario Corporativo – Tema claro + PDFs B/N + Login overlay
   ========================= */
...
const AUTH = {
  key:'inv.auth.v1',
  is(){ return !!store.get(this.key, false); },
...
$('#setPinBtn').onclick = ()=>{
  ...
  DB.save(db);
  alert('PIN guardado. Usa ese PIN para entrar.');
};
...
/* ---------- PDFs en B/N (mejorado) ---------- */
const { jsPDF } = window.jspdf || {};

/** Carga imagen, mantiene proporción (max mm) y convierte a gris */
function loadGrayImageDataUrl(src, maxWmm = 40, maxHmm = 16){
  return new Promise((resolve) => {
    const img = new Image();
    img.crossOrigin = 'anonymous';
    img.onload = () => {
      const mm2px = (mm)=>Math.floor(mm*3.78); // ~96dpi
...
/** Encabezado B/N con logo proporcional */
async function drawHeader(doc, title){
  ...
  if (db.settings.logoDataUrl) {
    const gray = await loadGrayImageDataUrl(db.settings.logoDataUrl, 40, 16);
    if (gray) {
      doc.addImage(gray.dataUrl, 'PNG', 14, 12, gray.wmm, gray.hmm);
      logoW = gray.wmm;
    }
  }
  const xText = 14 + (logoW ? (logoW + 6) : 0);
  const yBase = 18;

  doc.setFont('helvetica','bold'); doc.setFontSize(16);
  doc.text(db.settings.brandName || 'Inventario', xText, yBase);
  doc.setFont('helvetica','normal'); doc.setFontSize(11);
  doc.text(title, xText, yBase + 7);

  doc.line(14, 36, 196, 36);
}
function savePdf(doc, filename){ doc.save(filename.replace(/\s+/g,'_')+'.pdf'); }

/* PDF Catálogo */
$('#pdfItemsBtn')?.addEventListener('click', async ()=>{
  const doc = new jsPDF({unit:'mm', format:'a4', compress:true});
  await drawHeader(doc, 'Catálogo de Productos (B/N)');
...
/* PDF Stock por Almacén */
$('#pdfStockBtn')?.addEventListener('click', async ()=>{
  ...
  const doc = new jsPDF({unit:'mm', format:'a4', compress:true});
  await drawHeader(doc, 'Existencias por Almacén (B/N)');
  doc.autoTable({
    startY:42,
...
/* PDF Kardex por SKU */
$('#pdfKardexBtn')?.addEventListener('click', async ()=>{
  const sku = ($('#kardexSku')?.value||'').toLowerCase();
  ...
  const doc = new jsPDF({unit:'mm', format:'a4', compress:true});
  await drawHeader(doc, `Kardex – SKU: ${sku.toUpperCase()} (B/N)`);
